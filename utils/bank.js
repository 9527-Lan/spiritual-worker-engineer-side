...
const resultInfo = (falg,msg,data)=>{
    return {
        'falg':falg,
        'msg': msg,
        'data':data
...
const getBankNameByBankCode = (bankcode)=> {
...
const getBankInfoFromAli = (cardNum,resultFunc)=>{
    
    // console.log('--------调用阿里api----------')
    uni.showLoading({})
    let url = "https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=" + cardNum + "&cardBinCheck=true"
    uni.request({
        url: url,
        method: 'GET',
        // // data: params,
            ...
            if(res.statusCode == 200){
                ...
                if(res.data.validated){
                    let result = {
                        ...
                        'bankName':getBankNameByBankCode(res.data.bank),
                        'cardType':getCardTypeName(res.data.cardType)
                    }
                    
                     resultFunc(resultInfo(true,'验证成功',result))
                }else{
                     resultFunc(resultInfo(false,'请检查银行号是否正确！',res))
                }
        ...
        fail(error) {
           resultFunc(resultInfo(false,'请检查银行号是否正确！',error))
        },
        complete: info => {
            uni.hideLoading()
        }
...
const getBankCardInfo = (cardNum,resultFunc) => {
 
    //1.根据本地正则获取银行卡信息是否存在
    let result = {}
    for (let bankcard of bankcardList) {
        // console.log(bankcard);
        for (let regPatterns of bankcard.patterns) {
            if(regPatterns.reg.test(cardNum)){
                return resultFunc(resultInfo(true,'',{
                    'bankCode':bankcard.bankCode,
                    'bankName':bankcard.bankName,
                    'cardType':getCardTypeName(regPatterns.cardType)
                })) 
            }
        }
    }
    
    //2.如果没有进行 al 银行卡接口调用验证  返回信息
    getBankInfoFromAli(cardNum, resultFunc)
}
 
const _getBankBin = (cardNum, resultFunc) =>{
    
    let errMsg = '';
    if (isNaN(cardNum)) {
      cardNum = parseInt(cardNum);
      if (isNaN(cardNum)) {
        errMsg = cardNum + ':银行卡号必须是数字';
         return resultFunc(resultInfo(false,errMsg))
      }
    }
    if (cardNum.toString().length < 15 || cardNum.toString().length > 19) {
      errMsg = cardNum + ':银行卡位数必须是15到19位';
      return resultFunc(resultInfo(false,errMsg))
    }
    
    // console.log('执行了。。。')
    
    getBankCardInfo(cardNum, resultFunc);
 
}
 
const getBankBin = (cardNum) =>{
    return new Promise((resolve, reject) => {
        console.log('--------银行卡---------卡号：'+ cardNum);
        _getBankBin(cardNum,result => {
            result.falg ? resolve(result) : reject(result)
        })
